...
function main() {
  ...
  util.get({uri: argv.allxml, 'outFile': fnameAllXML},
    function (err, obj) {
  ...
  function extractDatasets(json) {
    ...
    for (let dataset of datasets) {
      ...
      for (skip of skips) {
        ...
        if (re.test(id) == true) {
          util.log("Note: Skipping " + id + " b/c matches regex '" + skip + "' in skips.");
          omit = true;
          break;
        }
      }
    ...
    util.log.debug("idregex = " + argv.idregex + " kept " + CATALOG["datasets"].length + "/" + datasets.length + " datasets.");
    ...
    if (CATALOG["datasets"].length == 0) {
      util.error(null, `Regex '${argv.idregex}' did not match and dataset ids.`, true);
    }
  }
}
...
function masters(CATALOG) {
  ...
  for (dataset of datasets) {
    ...
    let baseDir = argv.cachedir + "/" + dataset['id'].split("_")[0] + "/" + dataset['id'] + "/";
  ...
  function finished(ididx, fnameCDFML, body, fromCache) {
    ...
    if (body && !body['CDF']) {
      ...
      if (cdfVariables.length > 1) {        
        util.error(datasets[ididx]['id'], ["Case of more than one cdfVariable not implemented. Omitting.", cdfVariables], true);
      }
      ...
      for (let [idx, variable] of Object.entries(cdfVariables['variable'])) {
        if (variable['cdfVarData']['record'].length > 2) {
          cdfVariables["variable"][idx]['cdfVarData']['record'] 
              = cdfVariables["variable"][idx]['cdfVarData']['record'].slice(0,2)
        }
      }
...
function finalizeCatalog(CATALOG) {
  ...
  for (let dataset of datasets) {
    ...
    if (/\s\s.*|\( | \)/.test(dataset['title'])) {
      util.warning(dataset['id'], "Check title formatting: '" + dataset['title'] + "'");
    }
    ...
    if (dataset['info']['cadence']) {
      ...
      if (cadence !== undefined) {
        util.warning(dataset['id'], "Assumed TIME_RESOLUTION = " + dataset['info']['cadence'] + " => " + cadence);
        dataset['info']['cadence'] = cadence;
      } else {
        util.warning(dataset['id'], "Could not parse TIME_RESOLUTION: " + dataset['info']['cadence'] + " to use for cadence.");
      }
    } else {
      util.warning(dataset['id'], "No TIME_RESOLUTION found to use for cadence.");
    }
    ...
    if (dataset['info']['x_creationDate']) {
      ...
      if (creationDate) {
        util.warning(dataset['id'], "Assumed GENERATION_DATE = " + creationDateo + " => " + creationDate);
        dataset['info']['x_creationDate'] = creationDate;
      } else {
        util.warning(dataset['id'], "Could not parse GENERATION_DATE = " + creationDateo);
      }
    }
    ...
    for (let name of Object.keys(parameters)) {
      ...
      if (parameter['_vAttributesKept']['_DEPEND_2'] === null) {
        util.error(dataset['id'], name + " has an un-handled DEPEND_2. Omitting dataset.", false);
        dataset = null;
        break;
      }
      ...
      if (parameter['_vAttributesKept']['_DEPEND_1']) {
        ...
        if (Array.isArray(depend1)) {
          extractCoordinateSystemNameAndVectorComponents(dataset['id'], parameter, depend1);
        } else {
  ...
  function writeFiles() {
    ...
    for (let dataset of datasets) {
      ...
      fnameInfo = argv.cachedir + '/' + dataset['id'].split("_")[0] + '/' + dataset['id'] + '-info.json';
    ...
    if (datasets.length > 10) {
      util.log(`Wrote:   ... ${(datasets.length - 11)} info and info-full files.`);
    } else {
...
function extractParameterAttributes(dataset) {
  ...
  for (let [idx, variable] of Object.entries(cdfVariables)) {
    ...
    if (vAttributesKept['_VAR_TYPE'] === "data") {
      parameters[variable['name']]['type'] = cdftype2hapitype(variable['cdfVarInfo']['cdfDatatype']);
    }
...
function extractDepend1(dsid, depend1Variable) {
  ...
  if (!['CDF_CHAR', 'CDF_UCHAR'].includes(DEPEND_1_TYPE)) {
    ...
    if (['CDF_FLOAT', 'CDF_DOUBLE', 'CDF_REAL4', 'CDF_REAL8'].includes(DEPEND_1_TYPE)) {
      ...
      util.error(null, "Un-handled DEPEND_1 type for bins variable " + depend1Variable['name'] + DEPEND_1_TYPE, true);
    }
...
function extractCoordinateSystemNameAndVectorComponents(dsid, parameter, depend1) {
  ...
  if (foundComponents) {
    util.warning(dsid, parameter['name'] + ": Assumed DEPEND_1 = [" + depend1.join(", ") + "] => vectorComponents = [x, y, z]")
  ...
  if (foundComponents) {
    ...
    for (let knownName of knownNames) {
      if (parameter['name'].includes(knownName)) {
        foundName = true;
        util.warning(dsid, parameter['name'] + ": Assumed parameter name = " + parameter['name'] + " => coordinateSystemName = " + knownName);
        parameter['coordinateSystemName'] = knownName;
      }
    }
    if (foundName == false) {
      util.warning(dsid, "Could not infer coordinateSystemName from parameter name = " + parameter['name']);
    }
  }
}
