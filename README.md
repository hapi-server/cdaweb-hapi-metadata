...
This respository was developed to improve the HAPI metadata served at https://cdaweb.gsfc.nasa.gov/hapi. Metadata-related code is discussed in Section 2. Output files are available at [GitHub](https://github.com/hapi-server/cdaweb-hapi-metadata-files) and [mag.gmu.edu](http://mag.gmu.edu/git-data/cdaweb-hapi-metadata-data/).
...
* scripts for a fourth method that uses the [CDAS REST service](https://cdaweb.gsfc.nasa.gov/WebServices/REST/). The script that generates metadata is `CDAS2HAPIinfo.js` ($\sim$1000 lines). The output files generated by this script are placed in `../cdaweb-hapi-metadata-files`. Output files are available at [GitHub](https://github.com/hapi-server/cdaweb-hapi-metadata-files) and http://mag.gmu.edu/git-data/cdaweb-hapi-metadata-data/. (The input files used to create the final output are stored in `cache/bw`). 
...
1. `bw`, which uses the new code in this repository and generated HAPI metadata (stored at [GitHub](https://github.com/hapi-server/cdaweb-hapi-metadata-files) and [mag.gmu.edu](http://mag.gmu.edu/git-data/cdaweb-hapi-metadata-data/)).
...
2. `nl`, which uses an approach similar to the above for datasets with virtual variables and an approach similar to `jf` below otherwise. Output files are available at [mag.gmu.edu](http://mag.gmu.edu/git-data/cdaweb-hapi-metadata-data/hapi/nl).
...
3. `bh`, which uses [SPASE](https://spase-group.org/) records. Output files are available at [mag.gmu.edu](http://mag.gmu.edu/git-data/cdaweb-hapi-metadata-data/hapi/bh).
...
4. `jf`, which uses [master CDFs](https://cdaweb.gsfc.nasa.gov/pub/software/cdawlib/0MASTERS/), raw CDF files, and code from [Autoplot](https://sourceforge.net/p/autoplot/code/HEAD/tree/). Output files are available at [mag.gmu.edu](http://mag.gmu.edu/git-data/cdaweb-hapi-metadata-data/hapi/jf).
...
1. `node CDAS2HAPIinfo.js --keepids '^AC_'`, which creates HAPI metadata that is written to `../cdaweb-hapi-metadata-files`.
