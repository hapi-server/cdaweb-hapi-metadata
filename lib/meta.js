  ...
  function createDatasets(json) {
    ...
    for (let dataset_allxml of datasets_allxml) {
      ...
      if (dataset_allxml['mastercdf']) {
        ...
        let emsg = "(CDAWeb) No mastercdf given in all.xml";
...
function getVariableData(CATALOG) {
 ...
 for (let dataset of CATALOG['datasets']) {
  ...
  function requestData(dataset) {
    ...
    let stop  = dataset['_files1Last']['StopTime'];
    stop = util.incrementTime(stop, 0, 'seconds'); // To format

    let dt = new Date(stop).getTime() - new Date(start).getTime();
    // Some files, e.g., GOES8_EPHEMERIS_SSC are > 4 GB and have
    // data over full time range.
    if (dt > 86400*10) {
    ...
    if (dataset['_files1Last']['MimeType'] !== 'application/x-cdf') {
    ...
    util.get(reqOpts, function (err, obj) {
      if (err) {
        dataset['_dataError'] = ["Problem with url " + url1, err, "Request time range derived from", dataset['_files1Last']];
        util.error(id, dataset['_dataError'], false, logExt);
        finished();
        return;
      }
      if (!obj['FileDescription']) {
        dataset['_dataError'] = [
            "Problem with response from",
            url1,
            "No FileDescription node.",
...
function getSPASERecords(CATALOG) {
  ...
  for (let dataset of datasets) {
    ...
    if (dataset['_masters']) {
      if (!dataset['_masters']['json']['CDFglobalAttributes']) {
        ...
        if (resourceID1 && !resourceID1.startsWith('spase://')) {
          emsg1 = `spase_DatasetResourceID in Master JSON '${resourceID1}' does not start with 'spase://'.`;
          resourceID1 = undefined;
        } else {
          emsg1 = "No spase_DatasetResourceID in Master JSON.";
        }
      }
    }
    ...
    if (dataset['_data']) {
      ...
      if (cdfml) {
        ...
        if (resourceID2 && !resourceID2.startsWith('spase://')) {
          emsg2 = `SPASE_DATASETRESOURCEID in Master JSON '${resourceID1}' does not start with 'spase://'.`;
          resourceID2 = undefined;
        } else {
          emsg2 = "No SPASE_DATASETRESOURCEID in Master JSON.";
        }
      }
    }
    ...
    if (resourceID1 && resourceID2) {
      if (resourceID1 !== resourceID2) {
        // TODO. How to propagate this error up?
      }
    }
    let emsg = emsg1 + " " + emsg2 + " " + emsg3;
    if (!resourceID1 && !resourceID2) {
  ...
  function getSPASERecord(dataset, url, outFile) {
    ...
    util.get(reqObj, (err, obj) => {
      let emsg = "";
      if (err) {
        emsg = [url, err.message];
        util.error(dataset['id'], emsg, false, logExt);
        obj = null;
      }
      if (obj !== null && !obj['json']) {
        emsg = `(SPASE) Problem with ${url}. HTML returned?`;
        obj = null;
        util.rmSync(outFile);
      }
