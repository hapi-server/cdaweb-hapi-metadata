  ...
  for (let dataset of CATALOG['datasets']) {
    ...
    util.get({"uri": url,
      ...
      (err, obj) => {
        if (err) {
          util.error(id, [url, 'Error message', err], false, logExt);
          dataset['_inventoryError'] = JSON.stringify(err, null, 2);
          finished();
        }
    ...
    util.get({"uri": urlCDF, "id": id, "outFile": fnameMasterCDF, "parse": false},
      (err) => {
        if (err) {
          ...
          dataset['_masters']['cdfError'] = [urlCDF, 'Error message', JSON.stringify(err, null, 2)];
          finished();
          return;
        }
 ...
 for (let dataset of CATALOG['datasets']) {
  ...
  function requestData(dataset) {
    ...
    util.get(reqOpts, function (err, obj) {
      if (err) {
        dataset['_dataError'] = ["Problem with url " + url1, err];
        util.error(id, dataset['_dataError'], false, logExt);
        finished();
        return;
      }
      if (!obj['FileDescription']) {
        dataset['_dataError'] = ["Problem with url " + url1, "No FileDescription node."];
        util.error(id, dataset['_dataError'], false, logExt);
        finished();
        return;
      }
      ...
      util.get(reqOpts, function (err, obj) {
        if (err) {
          dataset['_dataError'] = ["Problem with url2 " + url2 + " referenced in " + url1, err];
          util.error(id, dataset['_dataError'], false, logExt);
          finished();
          return;
        }
  ...
  for (let dataset of datasets) {
    ...
    if (dataset['_data']) {
      ...
      if (resourceID === "" && cdfml) {
        ...
        if (!resourceID) {
          emsg += "(SPASE) No SPASE_DATASETRESOURCEID in sample CDF file. ";
          finished(dataset, null, emsg);
          continue;
        }
      }
    } else {
      emsg += "(CDAWeb) No sample CDF file available to search for SPASE_DATASETRESOURCEID. ";
    }

    if (resourceID !== "" && !resourceID.startsWith('spase://')) {
      emsg += `(CDAWeb) SPASE recourse ID of '${resourceID}' does not start with 'spase://'.`;
      finished(dataset, null, emsg);
      continue;
    }
