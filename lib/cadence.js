...
function extractCDFCadence(dataset, _data) {
  ...
  let cadenceSPASE = spase.extractSPASECadence(dsid, dataset['_spase'], "Cadence");
...
module.exports.extractCDFCadence = extractCDFCadence;
...
function inferCadence(timeRecords, dsid, MONTON) {
  ...
  if (dthist.length > 0) {
    ...
    if (dthist[0][0] <= 0) {
      if (monoton) {
        let emsg = `  Error:   (CDAWeb) Time records not monotonically `
        emsg += `increasing but MONOTON = '${MONTON}'.`;
        util.error(dsid, emsg, false, false);
      } else {
        util.error(dsid, "  Error:   Time records not monotonic.", false, false);
      }
    }
  }
  return cadenceData;
